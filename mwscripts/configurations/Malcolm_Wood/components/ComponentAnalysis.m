...
classdef ComponentAnalysis < handle
    properties
        components = {};
        componentNames;
        componentGraph;
    ...
    methods
        ...
        % Load and analyze all component XML files.
            obj.components = {};
            obj.componentNames = {};
            obj.componentGraph = digraph;
            tic;
            if isempty(obj.componentNames)
                obj.allComponentNames;
            end
            obj.componentGraph = obj.componentGraph.addnode(obj.componentNames);
            ...
            tic;
            obj.componentGraph = obj.createDigraph;
            t = toc;
            fprintf('Created graph in %f seconds\n',t);
        end
        
        function g = createDigraph(obj)
            g= digraph;
            g = g.addnode(obj.componentNames);
            for i=1:numel(obj.componentNames)
                d = obj.components{i}.dependsOn;
                [~,n] = slcellmember(d,obj.componentNames);
                g = g.addedge(i,n);
            end
        end
        ...
        % Returns all the components which are upstream of the named one.
            if iscell(componentname)
                compnames = {};
                for i=1:numel(componentname)
                    if ~slcellmember(componentname{i},compnames)
            ...
            us = obj.componentGraph.dfsearch(componentname);
            compnames = sort([{componentname};us]);
        end
        ...
        %   already been printed in previous branches.
            ...
            for i=1:numel(c.dependsOn)
                ...
                elseif mt_cellmember(sc,stop_at)
                    ...
                    if mt_cellmember(target,obj.allUpstreamComponents(sc))
